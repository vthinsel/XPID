...
MenuItem menu_runtime = MenuItem("Runtime");
...
MenuItem menu_deadzone = MenuItem("DeadZone");
MenuItem menu_readanalog = MenuItem("ReadAnalog");
      ...
      if (lcd_key != lcd_oldkey)   // if keypress is detected
      {
        ...
        if (lcd_key != lcd_oldkey)
        {
          ...
          Serial.println(lcd_key);
            if (lcd_key != btnNONE){LCDruntime = 0;}
          {
            switch (lcd_key) {
              case btnUP:
                menu.moveUp();
