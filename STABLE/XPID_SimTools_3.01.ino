...
void InitEEValues() //Used to initialize variables and EEPROM content in case of new arduino, or updated EEPROM schema
{
  ...
  */
  ...
  if (DeadZone == 0) {
    DeadZone = 5 ; //increase this value to reduce vibrations of motors
  }
  if (ReadAnalog == 0) {
    ReadAnalog = 8 ;
  }
...
void PrintEEValues() {
  ...
  Serial.print("derivative2:"); Serial.print(derivative2); Serial.print(" ");
  Serial.print("DeadZone:"); Serial.print(DeadZone); Serial.print(" ");
  Serial.print("ReadAnalog:"); Serial.print(ReadAnalog); Serial.print(" ");
}
...
void setup()
{
  ...
  //}
  ReadEEValues(); // in case of EEPROM upgrade, we get the old values to not overwrite them in the process
  if (EEPROM.read(addrEEInit) != VERSION) {
